...
model3_filename = os.path.join(dir_path, 'models', 'RF_model3.pickle')
...
def predict_results(dataset, model):
    ...
    print(y_pred.shape)
    pred_df['Failure_Prediction'] = y_pred
    pred_df = pred_df.sample(frac=1).reset_index(drop=True)
    
    print(pred_df.head())
    pred_df.to_csv(output_file, index= False)
    response = {'status': True, 'pred_df' : json.loads(pred_df.to_json(orient='records'))}
    return response
